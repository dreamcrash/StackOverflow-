...
if [[ $# -ne 5 ]]; then
    	...
    	echo "Usage: $0 <Keycloak Host> <Admin User Name> <Admin Password> <Realm Name> <Role JSON Representation>" >&2
        exit 1
fi
...
curl -k -sS	-X POST "http://$KEYCLOAK_HOST/auth/admin/realms/$REALM_NAME/roles" \
